...
const updateMiniCenteral = async (req, res) => {
    ...
    const { _id, company, title, describe, stock, price, BTU_output, efficiency_factor, energy_rating, working_current, CFM, Pa, pipe_connection, in_size, out_size, quiet, wifi, speeds, air4d, sabbath_command, onof_auto } = req.body
    if (!_id) {
        return res.status(400).json({ message: "all details are required" })
    }
