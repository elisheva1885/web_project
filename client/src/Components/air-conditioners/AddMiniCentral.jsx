...
const AddMiniCentral = () => {
    ...
    const onSubmit = async (data) => {
        ...
        // try {
            // const payload = {
            //     company: data.company, // Reference to the company ID
            //     title: data.title,
            //     describe: data.describe,
            //     imagepath: data.imagepath,
            //     stock: data.stock || 1, // Default to 1 if not provided
            //     price: data.price,
            //     BTU_output: {
            ...
            //     speeds: data.speeds
            //     // Excluding fields that are commented out in the schema
            // };
            try{
            const response = await axios.post('http://localhost:8000/api/air-conditioner/miniCenteral', data);
            setFormData(data);
            setShowMessage(true);
        } catch (error) {
            console.error(error);
