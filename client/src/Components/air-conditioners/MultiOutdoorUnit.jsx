...
const MultiOutdoorUnit = () => {
  ...
  const getMultiOutdoorUnit = async () => {
    ...
    setError(null);
    try {
      const res = await axios.get(`http://localhost:8000/api/air-conditioner/multiOutdoorUnit/multiOutdoorUnit/${productId}`);
      if (!res.status === 200) {
          ...
          <StepperPanel header="תפוקה ונתונים טכניים">
            <table style={{ ...styles.table, direction: 'rtl' }}>
              <tbody>
                ...
                <TableRow label="מידות יחידת קונדנסור" value={<span style={{ direction: 'ltr', unicodeBidi: 'embed' }}>
                  {`${product.condenser_unit_dimensions?.width} x ${product.condenser_unit_dimensions?.depth} x ${product.condenser_unit_dimensions?.height}`}
                </span>} />
              </tbody>
            </table>
          </StepperPanel>
...
export default MultiOutdoorUnit;